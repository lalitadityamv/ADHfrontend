...
export default function ChatWindow({ chat, onUpdate, onMenuOpen }) {
  ...
  return (
    <main style={{ flex: 1, display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden", background: "#212121", minWidth: 0 }}>
      <div style={{ padding: "12px 16px", display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
        <button onClick={onMenuOpen} style={{ background: "none", border: "none", color: "#666", cursor: "pointer", padding: 4, display: window.innerWidth <= 768 ? "block" : "none" }}>
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
            <path d="M2 4h14M2 9h14M2 14h14" stroke="#888" strokeWidth="1.5" strokeLinecap="round"/>
          </svg>
        </button>
        <span style={{ fontSize: 14, color: "#666", flex: 1 }}>{chat.title}</span>
<button
  onClick={exportPDF}
...
<div style={{ display: "flex", alignItems: "center", gap: 6 }}>
  <span style={{ fontSize: 11, color: "#555", whiteSpace: "nowrap" }}>Tokens: {maxTokens}</span>
  <input
    type="range"
    min={256}
    max={50000}
    ...
    style={{ width: 80, accentColor: "#e05c2a", cursor: "pointer" }}
  />
</div>
        <select value={model} onChange={e => setModel(e.target.value)} style={{ fontSize: 12, background: "#2a2a2a", color: "#aaa", padding: "4px 10px", borderRadius: 20, border: "1px solid #333", cursor: "pointer", outline: "none" }}>
          <option value="accounts/fireworks/models/minimax-m3">MiniMax M3</option>
          <option value="accounts/fireworks/models/deepseek-v4-pro">DeepSeek V4</option>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "20px 0" }}>
        {isEmpty ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", gap: 16, width: "100%", textAlign: "center", padding: "0 20px" }}>
            <div style={{ width: 48, height: 48, borderRadius: 12, background: "#e05c2a", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, fontWeight: 700, color: "#fff" }}>A</div>
            <h1 style={{ fontSize: 24, fontWeight: 600, color: "#ececec", margin: 0 }}>How can I help you today?</h1>
            <p style={{ fontSize: 14, color: "#666", margin: 0 }}>Powered by AMD Instinct GPUs</p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, maxWidth: 560, marginTop: 8, width: "100%" }}>
              {[
                ...
                <button
                  ...
                  style={{ background: "#2a2a2a", border: "1px solid #333", borderRadius: 12, padding: "14px 16px", cursor: "pointer", textAlign: "left", transition: "background 0.15s" }}
          ...
          <div style={{ maxWidth: 680, margin: "0 auto", padding: "0 40px", width: "100%" }}>
            {chat.messages.map((msg, i) => (
              <MessageBubble key={i} message={msg} isLast={i === chat.messages.length - 1} loading={loading} />
            ))}
            <div ref={bottomRef} />
          </div>
        )}
